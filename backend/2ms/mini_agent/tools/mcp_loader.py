...
async def load_mcp_tools_from_config_async(config: dict[str, Any] | None) -> list[Tool]:
    """Load MCP tools from an in-memory config dictionary."""
    global _mcp_connections
    ...
    try:
        ...
        print(f"Error loading MCP config from memory: {e}")
        import traceback
