...
class Agent:
    ...
    async def _emit_event(self, event_type: str, payload: dict[str, Any]) -> None:
        """Emit structured runtime events for external observers."""
    ...
    def _supports_streaming(self) -> bool:
        """Return whether the configured LLM client exposes a streaming API."""
        return callable(getattr(self.llm, "stream_generate", None))

    async def _stream_llm_response(self, step_number: int, tool_list: list[Tool]) -> LLMResponse:
        """Consume LLM stream events while printing incremental terminal output."""
        ...
        async for event in self.llm.stream_generate(messages=self.messages, tools=tool_list):
            ...
            if event.type == "thinking_delta" and event.delta:
                ...
                print(f"{Colors.DIM}{event.delta}{Colors.RESET}", end="", flush=True)
                await self._emit_event("thinking_delta", {"delta": event.delta, "step": step_number})
                ...
                await self._emit_event("content_delta", {"delta": event.delta, "step": step_number})
        ...
        if printed_thinking or printed_content:
            print()
    ...
    async def run(self, cancel_event: Optional[asyncio.Event] = None) -> str:
        ...
        while step < self.max_steps:
            ...
            if self._check_cancelled():
                ...
                await self._emit_event("run_cancelled", {"message": cancel_msg})
                return cancel_msg
            ...
            used_streaming = self._supports_streaming()

            try:
                if used_streaming:
                    response = await self._stream_llm_response(step + 1, tool_list)
                else:
                ...
                await self._emit_event("run_failed", {"error": error_msg})
                return error_msg
            ...
            if response.thinking and not used_streaming:
                print(f"\n{Colors.BOLD}{Colors.MAGENTA}🧠 Thinking:{Colors.RESET}")
                print(f"{Colors.DIM}{response.thinking}{Colors.RESET}")
            if response.thinking:
                await self._emit_event("thinking", {"content": response.thinking, "step": step + 1})

            # Print assistant response
            if response.content and not used_streaming:
                print(f"\n{Colors.BOLD}{Colors.BRIGHT_BLUE}🤖 Assistant:{Colors.RESET}")
        ...
        await self._emit_event("run_failed", {"error": error_msg, "reason": "max_steps_exceeded"})
        return error_msg
